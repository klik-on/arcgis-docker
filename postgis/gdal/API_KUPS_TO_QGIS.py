...
# ========== KONFIGURASI LOGGING ==========
logging.basicConfig(
    level=logging.INFO,  # Ubah ke DEBUG jika butuh detail
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[
        logging.StreamHandler(),  # Output ke konsol
        logging.FileHandler("log_kups.log", encoding="utf-8")  # Simpan ke file log
    ]
)
...
try:
    ...
    logging.info(f"🧾 Jumlah titik: {len(features)}")
except Exception as e:
